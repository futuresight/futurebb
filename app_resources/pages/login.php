...
if (isset($_POST['form_sent'])) {
	$result = $db->query('SELECT password,id,activate_key,g_admin_privs,login_hash FROM `#^users` AS u LEFT JOIN `#^user_groups` AS g ON g.g_id=u.group_id WHERE username=\'' . $db->escape($_POST['username']) . '\' AND deleted=0') or error('Failed to check user', __FILE__, __LINE__, $db->error());
	list($password, $id, $key, $admin_privs, $hash) = $db->fetch_row($result);
